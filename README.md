...
注意：MPUパワーを必要とするので、低クロックの000実機だとめちゃくちゃ時間がかかります。060turbo / PhantomX / エミュレータを推奨します。
...
MP3CVxxx.ZIP をダウンロードして展開し、実行ファイルをパスの通ったディレクトリに置きます。
実行ファイルは3種類同梱されているので、環境に合わせてリネームするなどして対応してください。
MPU種別のチェックは特にしていないので、非対応のものを実行すると白帯が出る可能性があります。

 - MP3CONV.X ... 全機種対応
 - MP3CV030.X ... 68030以上対応
 - MP3CV060.X ... 68060専用
...
MP3デコードライブラリとして libmad 0.15.1b を利用させて頂いています。ライセンスは libmad のもの (GPLv2) に準じます。
...
* 0.8.2 (2026/03/09) ... 68000版も -msoft-float をつけてビルドするようにした
* 0.8.1 (2026/03/07) ... 68030/68060版は -msoft-float をつけてビルドするようにした
* 0.8.0 (2026/03/01) ... 68030専用バイナリ、68060専用バイナリを同梱した
